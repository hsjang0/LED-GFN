...
class DetailedBalanceTransitionBuffer(DetailedBalance):
    ...
    def train_step_get_grad(self, *batch, reward_exp=None, logr_scaler=None, ep = 1):
        self.model.train()
        ...
        if self.forward_looking:
            if not self.cfg.heuristic:
                reward_est = -logr_scaler(self.model_reward(gb,s,s_next))
                flows_next.masked_fill_(d, 0.) # \tilde F(x) = F(x) / R(x) = 1, log 1 = 0
                lhs = flows + log_pf # (bs,)
                rhs = flows_next + log_pb
                loss = (lhs - rhs + reward_est).pow(2)
                loss = loss.mean()
            else:
                flows_next.masked_fill_(d, 0.) # \tilde F(x) = F(x) / R(x) = 1, log 1 = 0
